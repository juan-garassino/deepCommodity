...
- **Phase 11 — macro-contextual forecaster (crypto-first).** A shared global-macro encoder (M2, net liquidity, DXY, total-crypto-cap) conditions per-asset price encoders → multi-horizon (weekly+daily) direction, trained jointly over BTC/ETH/SOL (`--model contextual`). Leakage-safe (publication-lag alignment + train-only norm). Ships in **shadow** with a Telegram regime alert; promoted to a live forecast input only after the walk-forward eval (`tools/eval_contextual.py`) beats the rule-based baseline (≥5% acc lift + non-negative PnL edge). Trains/infers off-box; equities + serving = v2.
